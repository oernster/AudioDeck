...
def flow(parent: QWidget, leaving: str, arriving: str) -> QWidget:
    ...
    Args:
        parent: The screen this line is drawn on.
        leaving: The version installed now.
        arriving: The version this setup file carries.
...
def choices(
    ...
    Args:
        parent: The window the screen belongs to.
        heading: The line naming what this run is for.
        lead: The muted line under it.
        options: Each choice with the note that explains it.
        location: Where the files will go, when that is worth saying.
        versions: The version leaving and the version arriving, when both apply.
    ...
    made.addSpacing(c.HEADING_GAP_PX)
    made.addWidget(label(screen, lead, "Lead"))
    made.addSpacing(c.LEAD_GAP_PX)
    ...
    for box, hint in options:
        made.addSpacing(c.OPTION_SPACING_PX)
        made.addWidget(option(screen, box, hint))
    made.addStretch()
    return screen


def message(parent: QWidget, heading: QLabel, lead: QLabel) -> QWidget:
    """A screen that only has something to say, such as the app being open."""
    screen, made = column(parent, centred=True)
    made.addWidget(heading)
    made.addSpacing(c.HEADING_GAP_PX)
    made.addWidget(lead)
    made.addStretch()
    return screen
...
def progress(
    ...
    made.addSpacing(c.LEAD_GAP_PX)
    made.addWidget(bar)
    made.addSpacing(c.TRACK_GAP_PX)
