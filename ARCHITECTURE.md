...
| A disabled control still shows what it is SET to | Disabled means unchangeable, never blank: while an operation runs the options describe what the run is doing, so a ticked box reading as unticked states the opposite of the truth at the one moment it cannot be corrected | `tests/installer/test_theme_states.py`, sampling the painted indicator rather than reading the sheet |
