...
src/
  ...
  presentation/         GUI layer (PySide6)
    ...
    widgets/           KeyboardNavigator (the explicit focus ring),
                       ...
                       inactive_tooltips (tooltips while another program
                       has focus)
...
| Tooltips over an inactive window are switched on application-wide | Qt Widgets shows a tooltip only over the active window unless that window carries `WA_AlwaysShowToolTips`; one event filter sets it on every top-level window as it is shown, dialogs included, rather than each window opting in. Both composition roots install it (`src/main.py` and `installer/app.py`), so the setup program imports this one module from the `src` package and nothing else from it |
